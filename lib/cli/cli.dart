...
class PackagexEnvironment {
  ...
  static bool get is_not_interactive {
    if (RegExp(r"virtual", caseSensitive: false).hashData(SystemInformation().get_model)) {
      return true;
    }
...
FutureOr<void> packagexCli(List<String> arguments_origins) async {
  ...
  if (command == "build") {
    ...
    await for (final streamBuild in packagex.build(
      ...
      directoryBuild: () {
        final String output_path = (args.after(["--output-directory", "-output-directory"]) ?? "").trim();
