...
class Packagex {
  ...
  Future<Map> request({
    ...
    if (RegExp(r"^(createPackage)$", caseSensitive: false)
        .hashData(jsonDart["@type"])) {}
    if (RegExp(r"^(buildPackage)$", caseSensitive: false)
        .hashData(jsonDart["@type"])) {
      packagex_api.BuildPackage buildPackage =
          packagex_api.BuildPackage(jsonData);

      return await build(
        packagexPlatform: (PackagexPlatform.values
                .getByString(buildPackage.platform ?? "current")
            as PackagexPlatform),
...
""";
    Future<void> createFolders({
      ...
      try {
        await File(p.join(directory.path, "DEBIAN", "postinst"))
            .writeAsString("""
#!/usr/bin/env sh
...
""");
      ...
      try {
        await File(p.join(directory.path, "usr", "share", "applications",
    ...
    await createFolders(
      directory:
          Directory(p.join(directory_package.path, "android", "packagex")),
      folders: [
        ["DEBIAN"],
        ["usr", "bin"],
        ["usr", "lib"],
    ...
    await createFolders(
      directory:
          Directory(p.join(directory_package.path, "windows", "packagex")),
      folders: [],
    );
    ...
    if (Platform.isAndroid) {
      await packagex_shell.shell(
        ...
        arguments: [
          "775",
          p.join(directory_package.path, "android", "packagex", "DEBIAN",
              "postinst"),
        ],
        runInShell: true,
        onStdout: (data, executable, arguments, workingDirectory, environment,
    ...
    if (Platform.isLinux) {
      await packagex_shell.shell(
        ...
        arguments: [
          "775",
          p.join(directory_package.path, "linux", "packagex", "DEBIAN",
              "postinst"),
        ],
        runInShell: true,
        onStdout: (data, executable, arguments, workingDirectory, environment,
      ...
      await packagex_shell.shell(
        ...
        arguments: [
          "775",
          p.join(
              directory_package.path, "linux", "packagex", "DEBIAN", "postrm"),
        ],
        runInShell: true,
        onStdout: (data, executable, arguments, workingDirectory, environment,
            includeParentEnvironment, runInShell, mode) {
  ...
  Future<Map> build({
    ...
    packagex_scheme.Pubspec pubspec =
        packagex_scheme.Pubspec(yaml_code.clone());
    if (pubspec["name"] == null) {
    ...
    File script_cli = File(p.join(directory_current.path, "bin",
        "${pubspec.packagex.dart_target ?? pubspec.name}.dart"));
    File script_app = File(p.join(directory_current.path, "lib",
        "${pubspec.packagex.flutter_target ?? "main"}.dart"));
    bool is_app = false;
    bool is_cli = false;
    ...
    Directory directory_build_packagex = Directory(
        path_output ?? p.join(directory_current.path, "build", "packagex"));
    ...
    if (packagexPlatform == PackagexPlatform.linux) {
      ...
      File file_app = File(p.join(directory_build_packagex.path,
          "${pubspec.packagex.flutter_name ?? pubspec.name}${(pubspec.packagex.is_without_platform_name == true) ? "" : "-linux"}.deb"));
      ...
      if (is_cli) {
        ...
        try {
          await packagex_shell.shell(
            ...
            runInShell: true,
            onStdout: (data, executable, arguments, workingDirectory,
                environment, includeParentEnvironment, runInShell, mode) {
              stdout.add(data);
            },
            onStderr: (data, executable, arguments, workingDirectory,
                environment, includeParentEnvironment, runInShell, mode) {
              stderr.add(data);
            },
          );
      ...
      if (is_cli) {
        await packagex_shell.shell(
          ...
          arguments: [
            ...
            script_cli.path,
            "-o",
            p.join(directory_build_packagex.path,
                "${pubspec.packagex.dart_name ?? pubspec.name}${(pubspec.packagex.is_without_platform_name == true) ? "" : "-cli-windows"}.exe"),
          ],
          workingDirectory: directory_current.path,
          onStdout: (data, executable, arguments, workingDirectory, environment,
              includeParentEnvironment, runInShell, mode) {
      ...
      if (!Platform.isMacOS) {
        return {
          "@type": "error",
          "message": "platform_not_supported",
          "description": "Package macos hanya bisa di perangkat macos saja !"
        };
      }
      if (is_cli) {
        await packagex_shell.shell(
          ...
          arguments: [
            ...
            "-o",
            p.join(directory_build_packagex.path,
                "${pubspec.packagex.dart_name ?? pubspec.name}${(pubspec.packagex.is_without_platform_name == true) ? "" : "-cli-macos"}"),
          ],
          workingDirectory: directory_current.path,
          onStdout: (data, executable, arguments, workingDirectory, environment,
              includeParentEnvironment, runInShell, mode) {
      ...
      if (is_app) {
        ...
        Directory directory_apk = Directory(p.join(
            directory_current.path, "build", "app", "outputs", "flutter-apk"));
        ...
        for (var i = 0; i < dirs.length; i++) {
          ...
          try {
            if (dir is Directory) {
              ...
              await dir.absolute.copy(p.join(
                  directory_build_packagex.path,
                  p.basename(dir.path).replaceAll(
                      RegExp("^(app)", caseSensitive: false),
                      "${pubspec.packagex.flutter_name ?? pubspec.name}")));
            }
          } catch (e) {}
        }
      }
      ...
      if (is_cli) {
        if (!Platform.isAndroid) {
          return {
            ...
            "message": "platform_not_supported",
            "description":
                "Package Android hanya bisa di perangkat android saja !",
          };
        }
        ...
        try {
          await packagex_shell.shell(
            ...
            arguments: [
              "775",
              p.join(path_android_package, "DEBIAN", "postinst")
            ],
            runInShell: true,
            onStdout: (data, executable, arguments, workingDirectory,
                environment, includeParentEnvironment, runInShell, mode) {
        ...
        try {
          await packagex_shell.shell(
            ...
            arguments: [
              "775",
              p.join(path_android_package, "DEBIAN", "postrm")
            ],
            runInShell: true,
            onStdout: (data, executable, arguments, workingDirectory,
                environment, includeParentEnvironment, runInShell, mode) {
        ...
        File file_app = File(p.join(directory_build_packagex.path,
            "${pubspec.packagex.flutter_name ?? pubspec.name}${(pubspec.packagex.is_without_platform_name == true) ? "" : "-android"}.deb"));
...
"""
      ...
      if (is_app) {
        ...
        Directory directory_build_web_canvaskit = Directory(
            p.join(directory_current.path, "build", "web", "canvaskit", "."));
        ...
        if (Platform.isWindows) {
          ...
          await packagex_shell.shell(
            ...
            arguments: [
              "-cf",
              p.join(directory_build_packagex.path,
                  "${pubspec.packagex.flutter_name ?? pubspec.name}${(pubspec.packagex.is_without_platform_name == true) ? "" : "-web"}.zip"),
              "*"
            ],
  ...
  Future<void> publish({
    ...
    Directory directory_build =
        Directory(p.join(directory_current.path, "build"));
    Directory directory_packagex =
        Directory(p.join(directory_build.path, "packagex"));
    ...
    List<FileSystemEntity> files = await Future(() async {
      return directory_packagex
          .listSync()
          .where((e) => [".deb", ".apk", ".msix"].contains(p.extension(e.path)))
          .where((element) {
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      if (fileSystemEntity is File) {
        ReleaseAsset? releaseAsset = releaseAssets.firstWhereOrNull(
            (element) => element.name == p.basename(fileSystemEntity.path));
